...
int main(int argc, char *argv[]) 
{
  ...
  if ( argc != 4 )
  {
    printf("usage: saxpy-ocl.x <a> <0=cpu/1=gpu> <OutputFile>\n");
    exit(1);
  }
  // Recuperer valeur param a
  float a = atof(argv[1]);
  printf("Le parametre est a = %f\n",a);
  int iter;
  int err;
  ...
  // Boucle sur la dimension qui va de 2^10 à 2^26.
  for(iter = 10; iter < 27; iter++)
  {
    int LIST_SIZE = 2<<(iter+1);
    ...
    // Creation d’un buffers memoire pour chaque vecteur
    ...
    cl_mem coeff_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY,sizeof(float), NULL, &ret);
    ...
    ret = clEnqueueWriteBuffer(command_queue, coeff_mem_obj, CL_TRUE, 0,sizeof(float), &a, 0, NULL, NULL);
    ...
    cl_kernel kernel = clCreateKernel(program, "saxpy_gpu", &ret);
    if (ret<0 )
    ...
    ret = clSetKernelArg(kernel, 3, sizeof(cl_mem), (void *)&coeff_mem_obj);

    // Execute the OpenCL kernel on the list
    size_t global_item_size[] = {LIST_SIZE}; // Process the entire lists
    size_t local_item_size[] = {256}; // Process in groups of 256
    // Event timer
